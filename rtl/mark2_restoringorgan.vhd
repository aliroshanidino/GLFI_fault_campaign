...
-- Module: Lightweight TMR Homeostatic Neuron (Model 3 - RESTORING ORGAN)
-- Features: 4-Segment PWL, V/U Datapath Simplex, Theta & Control Flags TMR
...
end light_tmr_neuron2;

architecture Behavioral of light_tmr_neuron2 is
    
    -- 1. Unprotected State Registers (Datapath)
    signal v_reg : signed(WIDTH-1 downto 0) := (others => '0');
    signal u_reg : signed(WIDTH-1 downto 0) := to_signed(-106496, WIDTH); 
    
    -- 2. Triplicated State Registers (Homeostasis Brain)
    signal theta_reg_A, theta_reg_B, theta_reg_C : signed(WIDTH-1 downto 0) := to_signed(576717, WIDTH);
    
    -- [RESTORING ORGAN] Triplicated Theta Voters
    signal voted_theta_A, voted_theta_B, voted_theta_C : signed(WIDTH-1 downto 0);

    -- 3. Unprotected Combinational Datapath
    ...
    -- Simplex Homeostasis magnitudes
    ...
    -- 4. Triplicated Control Logic (The Hardened Core)
    ...
    -- [RESTORING ORGAN] Triplicated Control Flags
    signal voted_spk_A, voted_spk_B, voted_spk_C       : std_logic;
    signal voted_beta_A, voted_beta_B, voted_beta_C    : std_logic;
    signal voted_fb_A, voted_fb_B, voted_fb_C          : std_logic;
    signal voted_vmax_A, voted_vmax_B, voted_vmax_C    : std_logic;
    signal voted_vmin_A, voted_vmin_B, voted_vmin_C    : std_logic;
    signal voted_umax_A, voted_umax_B, voted_umax_C    : std_logic;
    signal voted_umin_A, voted_umin_B, voted_umin_C    : std_logic;
    ...
    -- [RESTORING ORGAN] Protect the triplicated voters from synthesis optimization!
    attribute keep of voted_spk_A, voted_spk_B, voted_spk_C : signal is "true";
    attribute keep of voted_theta_A, voted_theta_B, voted_theta_C : signal is "true";

begin

    -- ------------------------------------------------------------------------
    -- [RESTORING ORGAN] TRIPLICATED VOTERS (No Single Point of Failure)
    -- ------------------------------------------------------------------------
    voted_theta_A <= (theta_reg_A and theta_reg_B) or (theta_reg_B and theta_reg_C) or (theta_reg_A and theta_reg_C);
    voted_theta_B <= (theta_reg_A and theta_reg_B) or (theta_reg_B and theta_reg_C) or (theta_reg_A and theta_reg_C);
    voted_theta_C <= (theta_reg_A and theta_reg_B) or (theta_reg_B and theta_reg_C) or (theta_reg_A and theta_reg_C);

    voted_spk_A <= (f_spk_A and f_spk_B) or (f_spk_B and f_spk_C) or (f_spk_A and f_spk_C);
    voted_spk_B <= (f_spk_A and f_spk_B) or (f_spk_B and f_spk_C) or (f_spk_A and f_spk_C);
    voted_spk_C <= (f_spk_A and f_spk_B) or (f_spk_B and f_spk_C) or (f_spk_A and f_spk_C);

    voted_vmax_A <= (f_vmax_A and f_vmax_B) or (f_vmax_B and f_vmax_C) or (f_vmax_A and f_vmax_C);
    voted_vmin_A <= (f_vmin_A and f_vmin_B) or (f_vmin_B and f_vmin_C) or (f_vmin_A and f_vmin_C);
    
    voted_fb_A <= (f_fb_A and f_fb_B) or (f_fb_B and f_fb_C) or (f_fb_A and f_fb_C);
    
    voted_umax_A <= (f_umax_A and f_umax_B) or (f_umax_B and f_umax_C) or (f_umax_A and f_umax_C);
    voted_umin_A <= (f_umin_A and f_umin_B) or (f_umin_B and f_umin_C) or (f_umin_A and f_umin_C);
    
    voted_beta_A <= (f_beta_A and f_beta_B) or (f_beta_B and f_beta_C) or (f_beta_A and f_beta_C);

    -- ------------------------------------------------------------------------
    -- DATAPATH (Simplex) - Taps from Domain 'A' Restoring Organ outputs
    -- ------------------------------------------------------------------------
    ...
    v_clamped <= V_MAX_HW when voted_vmax_A = '1' else
                 V_MIN_HW when voted_vmin_A = '1' else
                 v_next_raw;

    u_pre_clamp <= u_fast_bleed when voted_fb_A = '1' else u_next_raw;

    u_clamped <= U_MAX_HW when voted_umax_A = '1' else
                 U_MIN_HW when voted_umin_A = '1' else
                 u_pre_clamp;

    -- Simplex brake magnitude calculation
    simplex_theta_err <= voted_theta_A - THETA_REST;
    beta_brake_val <= shift_left(simplex_theta_err, 3);
    beta_brake <= beta_brake_val when voted_beta_A = '1' else (others => '0');

    u_jump_raw <= u_clamped + D_PARAM + beta_brake;
    u_jump_clamped <= U_MAX_HW when u_jump_raw > U_MAX_HW else u_jump_raw;

    -- ------------------------------------------------------------------------
    -- TRIPLICATED CONTROL-PLANE FLAGS
    -- ------------------------------------------------------------------------
    ...
    f_fb_A <= '1' when (v_next_raw < FAST_BLEED_TH) and (theta_reg_A > THETA_REST) and (u_reg > U_REST) else '0';
    f_fb_B <= '1' when (v_next_raw < FAST_BLEED_TH) and (theta_reg_B > THETA_REST) and (u_reg > U_REST) else '0';
    f_fb_C <= '1' when (v_next_raw < FAST_BLEED_TH) and (theta_reg_C > THETA_REST) and (u_reg > U_REST) else '0';
    ...
    -- ------------------------------------------------------------------------
    -- [RESTORING ORGAN] ISOLATED 1-CYCLE STATE UPDATE
    -- ------------------------------------------------------------------------
    process(Clk, Reset)
    begin
        if Reset = '1' then
            ...
            if Enable = '1' then
                ...
                -- ==========================================
                -- 1. Simplex Datapath Update (Taps Voter A)
                -- ==========================================
                Spike <= voted_spk_A;
                if voted_spk_A = '1' then
                    v_reg <= C_PARAM; 
                ...
                -- ==========================================
                -- 2. TMR Control Plane (Fully Isolated Domains)
                -- ==========================================
                -- Domain A (Listens ONLY to Voter A)
                if voted_spk_A = '1' then
